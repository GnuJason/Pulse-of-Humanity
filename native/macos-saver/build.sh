...
if [[ ! -d "$PROJECT_DIR/resources/screensaver" ]]; then
  echo "Bundled screensaver assets were not found. Run npm run build:native:assets first." >&2
  exit 1
fi
...
cp -R "$PROJECT_DIR/resources/screensaver" "$RESOURCES_DIR/screensaver"
